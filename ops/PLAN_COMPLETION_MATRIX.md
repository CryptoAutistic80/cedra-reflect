...
| Phase 1 — specification and model | PASS (local) | `docs/accounting-specification.md`; independent Python model; hand-authored vector; three independent fixed-seed Python/Move witnesses with 64 applied operations each; 27 AMM arithmetic boundary vectors; 60 Python tests; claim-backed dirty-tree gate completed 1,000,000 applied state changes from 1,071,570 attempts with 70,626 no-ops, 944 rejected attempts, 2,002 full audits, 1,024 holders, and digest `a40abf6fd8f4b91c7152ba8a63016ef2ef49d2be6c698fdb4dcd87f6c16d90e9` | Re-run from the exact reviewed clean release commit and preserve the provenance report; the successful dirty-tree run deliberately produced no release report |
| Phase 2 — reflection core | PASS (local) | `move/reflection-core`; immutable publication policy and one-time claim-backed mode initialization; fixed supply with no retained mint authority; exact backing views; package-only events; clean initial schema; 8 core tests and the full 118-test Move suite pass locally; independent local contract re-audit GO with no remaining Critical/High/Medium/Low finding | Clean verification-bound exact-address bundle, independent external human review, and finalized chain proof |
| Phase 3 — AMM and faucet | PASS (local) | `move/test-amm`; `move/test-assets`; AMM and integration tests cover deployment-scale claim-backed flow, wallet/LP exact-once rewards, faucet pause, explicit AMM tUSD funding, atomic four-party handoff, and atomic registration of the authenticated bootstrap LP; assets has no package-local tests but is exercised by integration; all three generated conformance witnesses and the arithmetic boundary vectors also pass | Clean verification-bound exact-address bundle, gas results, and finalized same-ledger chain reconciliation |
...
| Structurally suitable for mainnet hardening | OPEN (human) | Internal local evidence only; independent human source/bytecode review and disposition of all high/critical findings remain required |

## Quantitative gates

| Gate | Target | Current status | Required proof |
|---|---:|---|---|
| Reference-model operations | >= 1,000,000 | PASS (local dirty-tree run; clean record pending) | 1,000,000 applied / 1,071,570 attempts / 70,626 no-ops / 944 rejected / 2,002 audits / 1,024 holders / `automatic_materialization=false` / digest `a40abf6fd8f4b91c7152ba8a63016ef2ef49d2be6c698fdb4dcd87f6c16d90e9`; rerun at the reviewed clean release commit to emit the provenance-bound report |
...
1. Five explicit Cedra Testnet profiles and public candidate addresses exist,
   ...
   authorization. No clean verification-bound exact-address bundle exists yet;
   external account activation and gas funding may occur only after that bundle
   passes independent human review and remain separate from the nine operations.
